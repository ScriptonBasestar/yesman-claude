...
class BackgroundTaskRunner:
    ...
    async def _run_task_safely(self, task_name: str, task_func: Callable, interval: int):
        ...
        async def check_sessions():
            try:
                ...
                if self.last_data["sessions"] != data_hash:
                    ...
                    logger.debug(f"Session data updated and broadcast ({len(formatted_sessions)} sessions)")
                    
            except Exception as e:
                logger.error(f"Error monitoring sessions: {str(e)}")
                raise
        
        await self._run_task_safely("sessions", check_sessions, self.intervals["sessions"])
        ...
        async def check_health():
            try:
                ...
                formatted_health = {
                    ...
                    "categories": {
                        "build": {"score": health_data.get('build_score', 0), "status": self._get_status(health_data.get('build_score', 0))},
                        "tests": {"score": health_data.get('test_score', 0), "status": self._get_status(health_data.get('test_score', 0))},
                        "dependencies": {"score": health_data.get('deps_score', 0), "status": self._get_status(health_data.get('deps_score', 0))},
                        "security": {"score": health_data.get('security_score', 0), "status": self._get_status(health_data.get('security_score', 0))},
                        "performance": {"score": health_data.get('perf_score', 0), "status": self._get_status(health_data.get('perf_score', 0))},
                        "code_quality": {"score": health_data.get('quality_score', 0), "status": self._get_status(health_data.get('quality_score', 0))},
                        "git": {"score": health_data.get('git_score', 0), "status": self._get_status(health_data.get('git_score', 0))},
                        "documentation": {"score": health_data.get('docs_score', 0), "status": self._get_status(health_data.get('docs_score', 0))}
                    },
                ...
                if self.last_data["health"] != data_hash:
                    ...
                    logger.debug(f"Health data updated and broadcast (score: {formatted_health['overall_score']})")
        ...
        async def check_activity():
            try:
                ...
                if self.last_data["activity"] != data_hash:
                    ...
                    logger.debug(f"Activity data updated and broadcast ({active_days} active days)")
                    
            except Exception as e:
                logger.error(f"Error monitoring activity: {str(e)}")
                raise
        
        await self._run_task_safely("activity", check_activity, self.intervals["activity"])
            ...
            try:
                ...
                for name, state in self.task_states.items():
                    if state.last_run:
                        ...
                        logger.debug(f"Task {name}: last run {age:.1f}s ago, errors: {state.error_count}")
    ...
    def get_task_states(self) -> Dict[str, Dict]:
        ...
        for name, state in self.task_states.items():
            states[name] = {
                ...
                "interval": self.intervals.get(name.replace("monitor_", "").replace("_task", ""))
            }
        return states
