...
class LiveDocumentationGenerator:
    ...
    def _build_component_map(self) -> dict[str, dict[str, str]]:
        ...
        return {
            "content_capture": {
                ...
                "common_issues": ["High memory usage during large content processing", "Slow response times with complex content structures", "Session timeout during long capture operations"],
            },
            "claude_status_check": {
                ...
                "common_issues": ["Connection timeouts to Claude API", "Authentication failures", "Rate limiting errors"],
            },
            "prompt_detection": {
                ...
                "common_issues": ["False positive prompt detections", "High CPU usage during complex pattern matching", "Accuracy degradation with unusual prompt formats"],
            },
            "content_processing": {
                ...
                "common_issues": ["Memory leaks during large document processing", "Performance degradation with complex content", "Error handling for malformed content structures"],
            },
            "response_sending": {
                ...
                "common_issues": ["Network connectivity issues", "Message delivery failures", "Response formatting errors"],
            },
            "automation_analysis": {
                ...
                "key_metrics": ["response_time", "accuracy_rate", "cpu_usage"],
                "common_issues": ["Complex workflow analysis timeouts", "Resource intensive analysis operations", "Accuracy issues with edge case scenarios"],
            },
            "test_performance": {
                ...
                "common_issues": ["Test timeout issues", "Memory usage spikes during test execution", "Flaky test results affecting reliability"],
            },
        }
    ...
    async def _generate_api_documentation(self) -> str:
        ...
        if api_metrics:
            doc_content.extend(["### Current API Performance", "", "| Endpoint | Response Time (ms) | Status |", "|----------|-------------------|--------|"])
    ...
    async def _generate_component_documentation(self) -> str:
        ...
        for component_name, component_info in self.component_map.items():
            ...
            doc_content.extend([f"**Current Status:** {status_icon} {status_text}", ""])

            # Performance metrics
            if metrics:
                doc_content.extend(["#### Live Performance Metrics", "", "| Metric | Current | Average | Peak | Status |", "|--------|---------|---------|------|--------|"])
                ...
                for metric_name in component_info["key_metrics"]:
                    if metric_name in metrics:
                        ...
                        doc_content.append(f"| {metric_name.replace('_', ' ').title()} | {current_val:.1f} | {avg_val:.1f} | {max_val:.1f} | {status} |")
            ...
            for i, issue in enumerate(component_info["common_issues"], 1):
                doc_content.extend(
                    [f"{i}. **{issue}**", f"   - *Check:* Monitor {', '.join(component_info['key_metrics'])} metrics", "   - *Solution:* Refer to troubleshooting guide for automated fixes", ""]
                )
        ...
        if components_data:
            healthy_components = sum(1 for comp_metrics in components_data.values() if self._is_component_healthy(comp_metrics))
            ...
            doc_content.extend(["| Resource | Average Usage | Status | Recommendation |", "|----------|---------------|--------|----------------|"])
            ...
            for metric_name, avg_value in avg_metrics.items():
                ...
                doc_content.append(f"| {metric_name.replace('_', ' ').title()} | {avg_value:.1f} | {status} | {recommendation} |")
        ...
        doc_content.extend(
            [
                ...
                "*Next update: Automated refresh every 30 minutes*  ",
                "*For real-time metrics, visit the dashboard at http://localhost:1420*",
            ]
        )
    ...
    def _get_optimization_recommendation(self, metric_name: str, value: float) -> str:
        ...
        recommendations = {
            "response_time": {50: "Optimal", 100: "Consider caching", 200: "Optimize queries", float("inf"): "Critical - immediate attention needed"},
            "memory_usage": {5: "Efficient", 10: "Monitor growth", 20: "Investigate leaks", float("inf"): "Critical - restart recommended"},
            "cpu_usage": {30: "Efficient", 60: "Monitor load", 90: "Scale resources", float("inf"): "Critical - investigate bottlenecks"},
            "error_rate": {0.001: "Excellent", 0.01: "Investigate patterns", 0.05: "Fix critical issues", float("inf"): "Critical - system unstable"},
        }
    ...
    def _calculate_average_metrics(self, components_data: dict) -> dict[str, float]:
        ...
        return {metric_name: sum(values) / len(values) if values else 0 for metric_name, values in metric_totals.items()}
    ...
    async def _generate_troubleshooting_documentation(self) -> str:
        ...
        if active_alerts:
            doc_content.extend(["## 🚨 Current System Alerts", "", "| Severity | Component | Message | Time |", "|----------|-----------|---------|------|"])

            for alert in active_alerts[:10]:  # Show top 10 alerts
                time_str = time.strftime("%H:%M:%S", time.localtime(alert.timestamp))
                doc_content.append(f"| {alert.severity.value.upper()} | {alert.component} | {alert.message[:50]}... | {time_str} |")
        ...
        doc_content.extend(
            [
                ...
                "",
                f"*Last Updated: {current_time}*  ",
                "*Auto-refresh: Every 15 minutes or when alerts change*  ",
                "*For real-time troubleshooting, use the dashboard at http://localhost:1420*",
            ]
        )
    ...
    async def _generate_performance_documentation(self) -> str:
        ...
        if metrics:
            ...
            for component, comp_metrics in metrics.items():
                ...
                priority = "High" if rt > 200 or mem > 15 or cpu > 80 else "Medium" if rt > 100 or mem > 10 or cpu > 50 else "Low"

                doc_content.append(f"| {component} | {rt:.1f}ms | {mem:.1f}MB | {cpu:.1f}% | {priority} |")
        ...
        doc_content.extend(
            [
                ...
                "*Next analysis: Scheduled every hour*  ",
                "*For real-time metrics: http://localhost:1420/performance*",
            ]
        )
    ...
    async def _generate_deployment_documentation(self) -> str:
        ...
        doc_content = [
            ...
            "*For deployment support: Create GitHub issue with 'deployment' label*",
        ]
    ...
    async def _generate_error_documentation(self) -> str:
        ...
        doc_content = [
            ...
            "*For error-specific support: Include error code in GitHub issue title*",
        ]
    ...
    async def _generate_testing_documentation(self) -> str:
        ...
        if test_summary:
            doc_content.extend(
                ["### Current Test Suite Performance", "", "| Test Suite | Runs | Success Rate | Avg Duration | Status |", "|------------|------|--------------|--------------|--------|"]
            )

            for test_name, metrics in test_summary.items():
                success_rate = metrics["success_rate"] * 100
                avg_duration = metrics["avg_duration_ms"]
                status = "🟢 Good" if success_rate > 95 and avg_duration < 5000 else "🟡 Warning" if success_rate > 90 and avg_duration < 10000 else "🔴 Issues"

                doc_content.append(f"| {test_name} | {metrics['total_runs']} | {success_rate:.1f}% | {avg_duration:.0f}ms | {status} |")
        ...
        doc_content.extend(
            [
                ...
                "*Test metrics refresh: Every test run*  ",
                "*For testing issues: Create GitHub issue with 'testing' label*",
            ]
        )
