...
class DeploymentValidator:
    ...
    def __init__(self):
        ...
        self.checks = [
            ...
            DeploymentCheck(
                ...
                check_function=self._check_configuration_integrity,
                is_critical=True,
                timeout_seconds=15
            ),
            DeploymentCheck(
                name="resource_availability",
                description="System resources are available",
    ...
    async def validate_deployment(self) -> dict[str, Any]:
        ...
        await self._publish_validation_event("validation_started", {
            'total_checks': len(self.checks),
            'critical_checks': sum(1 for check in self.checks if check.is_critical)
        })
        ...
        await self._publish_validation_event("validation_completed", {
    ...
    async def _run_check(self, check: DeploymentCheck) -> dict[str, Any]:
        ...
        for attempt in range(check.retry_count):
            ...
            try:
                ...
                result['details'] = {
                    'error': f'Check timed out after {check.timeout_seconds} seconds',
                    'timeout': True
                }
    ...
    async def _check_configuration_integrity(self) -> dict[str, Any]:
        ...
        try:
            ...
            critical_missing = [r for r in config_results 
                              if r['file'] in critical_files and r['status'] != 'valid']
