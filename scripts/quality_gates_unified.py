...
class UnifiedQualityGatesChecker:
    ...
    def __init__(self, baseline_path: Optional[Path] = None):
        ...
        self.performance_checker = QualityGatesPerformanceChecker(
            self.baseline_path / "performance_baselines.json"
        )
    ...
    async def run_security_scan(self) -> SecurityMetrics:
        ...
        false_positive_rate = (
            (validator.false_positives / max(total_violations, 1)) * 100
    ...
    async def check_security_performance_correlation(
            ...
            for component, error_rate in error_rates.items():
                if error_rate > 5:  # 5% error rate
                    correlations.append({
                        ...
                        "severity": "medium",
                        "recommendation": self._get_correlation_recommendation(
                            "violation_error_correlation", error_rate
                        ),
                    })
        ...
        if security_baseline:
            ...
            if baseline_violations > 0:
                regression_percent = (
                    (security_metrics.violations_found - baseline_violations) / baseline_violations
                ) * 100
                ...
                if regression_percent > self.correlation_thresholds["security_regression_threshold"]:
                    correlations.append({
                        ...
                        "severity": "high",
                        "recommendation": self._get_correlation_recommendation(
                            "security_regression", regression_percent
                        ),
                    })
    ...
    def _get_correlation_recommendation(
        ...
        return recommendations.get(correlation_type, {}).get(
            severity,
            "Review correlation findings and take appropriate action."
        )
    
    def _load_security_baseline(self) -> Optional[Dict[str, Any]]:
    ...
    async def check_unified_quality_gates(
        ...
        result.passed = (
            ...
            len(result.blocking_failures) == 0
        )
    ...
    def _generate_recommendations(
        ...
        if not result.security_passed:
            recommendations.append(
                "🔒 Security: Address critical security violations before deployment"
            )
        
        security_metrics = result.metrics.get("security", {})
        if security_metrics.get("false_positive_rate", 0) > 20:
            recommendations.append(
                "🎯 Security: High false positive rate detected. Review security rules."
            )
        
        # Performance recommendations
        if not result.performance_passed:
            recommendations.append(
                "⚡ Performance: Optimize components exceeding performance thresholds"
            )
        ...
        if high_impact_correlations:
            recommendations.append(
                "🔗 Correlation: Security operations significantly impact performance. "
                "Consider optimization or scheduling adjustments."
            )
        ...
        if result.passed:
            ...
            recommendations.append(
                "❌ Quality gates failed. Address blocking issues before proceeding."
            )
