...
class EnhancedSecurityValidator:
                ...
                for line_num, line in enumerate(lines, 1):
                    ...
                    if re.search(pattern, line):
                        ...
                        if is_false_positive:
                            ...
                            await self._add_security_issue_async(
                                file_path, line_num, category, severity,
    ...
    async def _check_dangerous_calls_async(self, file_path: Path, node: ast.Call) -> None:
        ...
        if func_name in dangerous_calls:
            ...
            await self._add_security_issue_async(
                file_path, node.lineno, 'dangerous_function', severity,
        ...
        for alias in node.names:
            if alias.name in dangerous_imports:
                ...
                self.warnings.append(
                    f"{file_path}:{node.lineno}: Warning ({severity}): "
    ...
    async def _check_string_operations_async(self, file_path: Path, node: ast.BinOp) -> None:
        ...
        if isinstance(node.op, ast.Mod) and hasattr(node.left, 's'):
            ...
            if any(keyword in node.left.s.lower() 
                ...
                self.warnings.append(
                    f"{file_path}:{node.lineno}: Warning (high): "
                ...
                for line_num, line in enumerate(lines, 1):
                    if '=' in line and not line.strip().startswith('#'):
                        ...
                        if len(value) > 20 and not value.startswith('$'):
                            self.warnings.append(
                                f"{env_file}:{line_num}: Warning (medium): "
            ...
            try:
                ...
                for pattern, package_desc in vulnerable_patterns:
                    if re.search(pattern, content, re.IGNORECASE):
                        self.warnings.append(
                            f"{req_file}: Warning (high): Vulnerable dependency: {package_desc}"
                        )
                        self.violation_counts['high'] += 1
            except Exception:
                continue
            ...
            try:
                ...
                if 'production' in str(config_file).lower():
                    if re.search(r'debug["\s:=]+true', content.lower()):
                        self.errors.append(
                            f"{config_file}: CRITICAL: Debug mode enabled in production config"
                        )
                ...
                if re.search(r'http://[^localhost]', content):
                    self.warnings.append(
                        f"{config_file}: Warning (medium): Insecure HTTP protocol in config"
                    )
                    self.violation_counts['medium'] += 1
                ...
                if re.search(r'(password|secret)["\s:=]+["\']?(admin|password|123)', content.lower()):
                    self.errors.append(
                        f"{config_file}: HIGH: Weak or default credentials in config"
                    )
                    self.violation_counts['high'] += 1
            except Exception:
                continue
