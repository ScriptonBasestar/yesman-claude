...
def configure(output_dir, format, compression, buffer_size):
    ...
    try:
        ...
        # Test log entry
        async_logger.log(LogLevel.INFO, "Async logging system configured", extra_data={
            "config_test": True,
            "timestamp": time.time()
        })
...
def _display_log_statistics(console: Console, stats: Dict[str, Any]):
    ...
    overview = Panel(
        ...
        f"Time Range: Last {24} hours\\n"
        f"Error Count: {stats['level_counts']['ERROR'] + stats['level_counts']['CRITICAL']}",
        title="📊 Log Overview",
        border_style="blue"
    )
