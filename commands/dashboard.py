...
from libs.dashboard.renderers import (
    TUIRenderer, render_widget, WidgetType, RenderFormat
)
from libs.dashboard.renderers.widget_models import (
    SessionData, SessionStatus, MetricCardData
)
...
def check_dependencies(interface: str) -> Dict[str, bool]:
    ...
    if interface == "tauri":
        ...
        deps["tauri"] = (
            dashboard_path.exists() and
            (dashboard_path / "package.json").exists() and
...
def launch_web_dashboard(host: str = "localhost", port: int = 8080, 
                        theme: Optional[str] = None, dev: bool = False, 
    ...
    try:
        ...
        if result == 0:
            click.echo(f"Port {port} is already in use. Try a different port with -p option.", err=True)
            sys.exit(1)
        
        # Start FastAPI server
        api_path = Path(__file__).parent.parent / "api"
        if not api_path.exists():
            click.echo("Web dashboard API not found. Creating simple HTML interface...", err=True)
...
<head>
    ...
    <style>
        ...
        .status {{ margin-top: 20px; padding: 15px; background: #f0f9ff; border-radius: 6px; }}
    </style>
</head>
<body>
    <div class="container">
...
"""
            ...
            if detach:
                ...
                server_thread.start()
                click.echo(f"Web dashboard started in background at http://{host}:{port}")
...
def launch_tauri_dashboard(theme: Optional[str] = None, dev: bool = False, 
                          detach: bool = False) -> None:
...
@click.option('--interface', '-i', 
              ...
              default='auto', help='Dashboard interface to use')
@click.option('--host', '-h', default='localhost', help='Web dashboard host (web only)')
@click.option('--port', '-p', default=8080, type=int, help='Web dashboard port (web only)')
...
def run(interface, host, port, theme, dev, detach):
    ...
    try:
        if interface == 'tui':
            ...
            launch_web_dashboard(host=host, port=port, theme=theme, dev=dev, detach=detach)
...
def list_interfaces():
    ...
    interfaces = [
        ...
        ("tauri", "Desktop Application", "Native desktop app with Svelte frontend", deps['tauri'])
    ]
    
    for iface, name, description, available in interfaces:
        status = "✅ Available" if available else "❌ Not Available"
        click.echo(f"  {iface.upper():<6} | {name:<20} | {description:<40} | {status}")
...
@click.option('--interface', '-i', 
              ...
              default='tauri', help='Interface to build')
def build(interface):
