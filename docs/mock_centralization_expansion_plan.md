...
2. **ClaudeManager**: 9 duplications across 7 files  
3. **subprocess.run**: 15 duplications across 8 files
4. **TmuxManager**: 6 duplications across 4 files
5. **libtmux.Server**: 4 duplications across 4 files
...
2. **api/test_sessions.py** - SessionManager + API responses (6 occurrences)
3. **commands/test_dashboard.py** - ClaudeManager (4 occurrences)
4. **tests/unit/core/test_session_manager.py** - SessionManager (high complexity)
5. **tests/integration/test_full_automation.py** - Multiple managers
...
2. **Migrate**: Replace mock setup with factory/fixture
3. **Validate**: Ensure identical test behavior
...
3. **Start with lowest-risk migration** (commands/test_setup.py)
4. **Set up validation pipeline** for each migration step
...
This expansion plan provides a systematic approach to modernizing our test infrastructure while minimizing risk and maximizing developer productivity.