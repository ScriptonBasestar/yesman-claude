...
## 현상
`libs/yesman_config.py`의 `YesmanConfig` 클래스는 `yesman.yaml` 파일에서 설정을 읽어오는 기능(`get`)만 제공하며, 설정을 파일에 다시 저장하는 기능(`set` 또는 `save`)이 구현되어 있지 않습니다.
...
2.  **`YesmanConfig` 확장:** `yesman.yaml` 파일에 변경된 설정 내용을 저장하는 `save(config_data: dict)`와 같은 메서드를 `YesmanConfig` 클래스에 새로 추가해야 합니다. 이 작업은 별도의 태스크로 관리합니다.
