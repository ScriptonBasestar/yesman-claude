...
## 현상
`SessionManager` 클래스에는 세션을 설정(`setup`)하거나 제거(`teardown`)하는 메서드가 존재하지 않습니다. 따라서 `setup_tmux_session`과 `teardown_all_sessions` API 엔드포인트를 계획대로 구현할 수 없습니다.
...
2.  **계획 수정:** 2단계 계획을 "세션 **조회** 로직 이전"으로 축소하고, 세션 **생성/삭제** 기능은 `TmuxManager`를 분석한 후 별도의 단계(예: 2.5단계)에서 다시 다루도록 계획을 수정해야 합니다.
