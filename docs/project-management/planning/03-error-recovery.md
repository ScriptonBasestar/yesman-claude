...
Implement a comprehensive error recovery system that ensures yesman-claude can automatically recover from failures, maintain session state, and provide seamless user experience even during unexpected interruptions.
...
2. **Session Level**: Tmux session state preservation
3. **Application Level**: Transaction and rollback support
4. **Data Level**: Checkpoint and restore mechanisms
5. **System Level**: Health checks and self-healing
...
2. **Exponential Backoff**: For overload scenarios
3. **Circuit Breaker**: For cascading failures
4. **Checkpoint Restore**: For state loss
5. **Full Rebuild**: For complete corruption
