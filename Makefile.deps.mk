...
deps-upgrade: ## upgrade all dependencies to latest versions
	@echo "$(CYAN)Upgrading all dependencies to latest versions...$(RESET)"
...
deps-security: ## check dependencies for security vulnerabilities
	@echo "$(CYAN)Running security vulnerability check...$(RESET)"
...
deps-clean: ## remove unused dependencies
	@echo "$(CYAN)Cleaning unused dependencies...$(RESET)"
	@command -v pip-autoremove >/dev/null 2>&1 || pip install pip-autoremove
	@pip-autoremove -y
	@echo "$(GREEN)✅ Unused dependencies removed$(RESET)"

deps-freeze: ## freeze current dependencies
	@echo "$(CYAN)Freezing dependencies...$(RESET)"
	pip freeze > requirements.freeze.txt
	@echo "$(GREEN)✅ Dependencies frozen to requirements.freeze.txt$(RESET)"
...
reqs-generate: ## generate requirements files from pyproject.toml
	@echo "$(CYAN)Generating requirements files...$(RESET)"
...
reqs-dev: ## generate dev requirements
	@echo "$(CYAN)Generating dev requirements...$(RESET)"
	pip freeze | grep -E "(ruff|mypy|black|pytest|bandit)" > requirements-dev.txt
	@echo "$(GREEN)✅ Dev requirements generated$(RESET)"

reqs-test: ## generate test requirements
	@echo "$(CYAN)Generating test requirements...$(RESET)"
	pip freeze | grep -E "(pytest|pytest-|mock)" > requirements-test.txt
	@echo "$(GREEN)✅ Test requirements generated$(RESET)"
...
venv-activate: ## show how to activate virtual environment
	@echo "$(CYAN)To activate virtual environment:$(RESET)"
...
deps-info: ## show dependency information
	...
	@echo ""
	@echo "$(GREEN)📊 Dependency Statistics:$(RESET)"
...
deps-licenses: ## show dependency licenses
	@echo "$(CYAN)Dependency Licenses:$(RESET)"
...
deps-size: ## show dependency sizes
	@echo "$(CYAN)Dependency Sizes:$(RESET)"
	...
	@echo ""
	@echo "$(YELLOW)Top 10 largest packages:$(RESET)"
