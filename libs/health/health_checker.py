...
class LivenessTestError(HealthCheckError):
    """Exception raised when basic liveness tests fail."""
    pass
...
class HealthChecker:
    ...
    async def _get_session_manager(self) -> SessionManager | None:
        """Get session manager instance lazily."""
        if self.session_manager is None:
            try:
                self.session_manager = SessionManager()
            except Exception as e:
                logger.debug("SessionManager not available for health checks: %s", e)
    ...
    async def readiness_check(self) -> dict[str, Any]:
        ...
        try:
            ...
            service_checks = await asyncio.wait_for(self._check_critical_services(), timeout=self.readiness_timeout)
            ...
            await self._publish_health_event(
                "readiness_check", {"ready": overall_ready, "checks_count": len(checks), "response_time_ms": response_time, "failed_checks": [c.name for c in checks if c.status != "healthy"]}
            )
            ...
            result = {
                ...
                "checks": [{"service": check.name, "status": check.status, "response_time_ms": check.response_time_ms, "details": check.details} for check in checks],
            }
            ...
            if not overall_ready:
                ...
                if FASTAPI_AVAILABLE:
                    raise HTTPException(
                        503, {"error": "Service not ready", "failed_checks": [{"service": check.name, "status": check.status, "details": check.details} for check in failed_checks], **result}
                    )
                else:
    ...
    async def liveness_check(self) -> dict[str, Any]:
        ...
        try:
            ...
            await asyncio.wait_for(self._basic_liveness_tests(), timeout=self.liveness_timeout)

            response_time = (time.perf_counter() - start_time) * 1000

            # Publish liveness check event
            await self._publish_health_event("liveness_check", {"alive": True, "response_time_ms": response_time})

            return {"status": "alive", "timestamp": time.time(), "response_time_ms": response_time}
    ...
    async def health_metrics(self) -> dict[str, Any]:
        ...
        try:
            ...
            metrics_data = await asyncio.wait_for(self._collect_health_metrics(), timeout=self.metrics_timeout)

            response_time = (time.perf_counter() - start_time) * 1000

            # Publish metrics collection event
            await self._publish_health_event(
                "metrics_collection", {"collection_time_ms": response_time, "health_score": metrics_data.get("health_score", 0), "active_alerts": metrics_data.get("active_alerts", 0)}
            )

            return {**metrics_data, "collection_time_ms": response_time, "timestamp": time.time()}

        except TimeoutError:
            response_time = (time.perf_counter() - start_time) * 1000
            return {"status": "timeout", "timestamp": time.time(), "collection_time_ms": response_time, "error": f"Metrics collection timed out after {self.metrics_timeout}s"}

        except Exception as e:
            response_time = (time.perf_counter() - start_time) * 1000
            return {"status": "error", "timestamp": time.time(), "collection_time_ms": response_time, "error": str(e)}
    ...
    async def _check_critical_services(self) -> list[HealthCheckResult]:
        ...
        checks.append(
            HealthCheckResult(name="session_manager", status=session_status, response_time_ms=(time.perf_counter() - session_check_start) * 1000, details=session_details, timestamp=time.time())
        )
        ...
        try:
            ...
            monitoring_details = {"service": "monitoring", "health_score": health_score, "threshold": self.health_score_threshold}
        except Exception as e:
            monitoring_status = "unhealthy"
            monitoring_details = {"service": "monitoring", "error": str(e)}

        checks.append(
            HealthCheckResult(name="monitoring", status=monitoring_status, response_time_ms=(time.perf_counter() - monitoring_check_start) * 1000, details=monitoring_details, timestamp=time.time())
        )
        ...
        try:
            ...
            test_event = Event(type=EventType.CUSTOM, data={"event_subtype": "health_check_test"}, timestamp=time.time(), source="health_checker")

            # This would be a non-blocking publish for health check
            await asyncio.wait_for(self.event_bus.publish(test_event), timeout=1.0)
        ...
        checks.append(
            HealthCheckResult(name="event_bus", status=event_bus_status, response_time_ms=(time.perf_counter() - event_bus_check_start) * 1000, details=event_bus_details, timestamp=time.time())
        )
    ...
    async def _get_system_metrics(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                "cpu": {"percent": cpu_percent, "status": "normal" if cpu_percent < self.cpu_threshold else "high"},
                "memory": {
                ...
                "disk": {"percent": disk_info.percent, "total_gb": disk_info.total / (1024**3), "free_gb": disk_info.free / (1024**3), "status": "normal" if disk_info.percent < 85.0 else "high"},
...
if FASTAPI_AVAILABLE:
    ...
    async def health_status() -> dict[str, Any]:
        ...
        overall_healthy = readiness.get("status") == "ready" and liveness.get("status") == "alive" and metrics.get("status") in {"healthy", "degraded"}

        return {"overall_status": "healthy" if overall_healthy else "unhealthy", "readiness": readiness, "liveness": liveness, "metrics": metrics, "timestamp": time.time()}
