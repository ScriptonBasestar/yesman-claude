...
class UserFriendlyError:
    ...
    troubleshooting_guide_id: str | None = None
...
class ContextualErrorHandler:
    ...
    def _load_error_templates(self) -> dict[str, UserFriendlyError]:
        ...
        return {
            "SYS-001": UserFriendlyError(
                ...
                documentation_links=["/docs/generated/performance_guide.md", "/docs/generated/troubleshooting_guide.md#performance-issues"],
            ...
            "SYS-002": UserFriendlyError(
                ...
                documentation_links=["/docs/generated/troubleshooting_guide.md#memory-issues", "/docs/generated/performance_guide.md#memory-optimization"],
                auto_fix_available=True,
            ),
            "NET-001": UserFriendlyError(
                ...
                documentation_links=["/docs/generated/troubleshooting_guide.md#network-issues", "/docs/generated/api_reference.md#error-codes"],
                auto_fix_available=True,
            ...
            "NET-002": UserFriendlyError(
                ...
                documentation_links=["/docs/generated/troubleshooting_guide.md#network-issues"],
            ...
            "AUTH-001": UserFriendlyError(
                ...
                documentation_links=["/docs/generated/api_reference.md#authentication", "/docs/generated/troubleshooting_guide.md#authentication-failures"],
                auto_fix_available=False,
            ),
            "AUTH-002": UserFriendlyError(
                ...
                suggested_actions=["Click 'Log In' to authenticate again", "Your work will be saved automatically", "Consider enabling 'Remember Me' for longer sessions"],
                documentation_links=["/docs/generated/api_reference.md#authentication"],
                auto_fix_available=False,
            ),
            "CFG-001": UserFriendlyError(
                ...
                documentation_links=["/docs/configuration.md", "/docs/generated/troubleshooting_guide.md#configuration-issues"],
                auto_fix_available=True,
            ),
            "DB-001": UserFriendlyError(
                ...
                documentation_links=["/docs/generated/troubleshooting_guide.md#database-problems"],
                auto_fix_available=True,
            ),
            "VAL-001": UserFriendlyError(
                ...
                documentation_links=["/docs/user-guide.md#data-formats"],
                auto_fix_available=False,
            ),
    ...
    def _load_error_patterns(self) -> dict[str, dict[str, Any]]:
        ...
        return {
            ...
            "timeout": {"keywords": ["timeout", "timed out", "deadline exceeded"], "exception_types": ["TimeoutError", "asyncio.TimeoutError"], "error_code": "NET-002"},
            ...
            "session_expired": {"keywords": ["session expired", "token expired", "login required"], "exception_types": ["SessionExpiredError"], "error_code": "AUTH-002"},
            # System resource patterns
            "memory": {"keywords": ["memory", "out of memory", "allocation failed"], "exception_types": ["MemoryError", "OutOfMemoryError"], "error_code": "SYS-002"},
            "performance": {"keywords": ["slow", "performance", "high load", "resource"], "exception_types": ["PerformanceError"], "error_code": "PERF-001"},
            # Database patterns
            "database": {
            ...
            "configuration": {"keywords": ["configuration", "config", "settings", "invalid format"], "exception_types": ["ConfigurationError", "ValueError"], "error_code": "CFG-001"},
            # Validation patterns
            "validation": {"keywords": ["validation", "invalid", "format", "required field"], "exception_types": ["ValidationError", "ValueError"], "error_code": "VAL-001"},
        }
    ...
    async def handle_error(self, exception: Exception, context: dict[str, Any] = None, request_id: str = None) -> UserFriendlyError:
        ...
        try:
            ...
            return UserFriendlyError(
                ...
                suggested_actions=["Try refreshing the page", "Contact support with the error details", "Include the time when this error occurred"],
                documentation_links=[],
                timestamp=time.time(),
            )
    ...
    def _create_generic_error(self, exception: Exception) -> UserFriendlyError:
        ...
        return UserFriendlyError(
            ...
            suggested_actions=["Try your request again", "If the problem persists, please contact support", "Include the error details and time when reporting"],
            documentation_links=["/docs/generated/troubleshooting_guide.md", "/docs/generated/error_reference.md"],
            auto_fix_available=False,
        )

    async def _enhance_error_with_context(self, error: UserFriendlyError, context: dict[str, Any] = None, exception: Exception = None) -> UserFriendlyError:
        ...
        context_summary = f"Health: {system_context.get('health_score', 100)}%, Alerts: {active_alerts}, Time: {time.strftime('%H:%M:%S')}"
    ...
    async def _get_system_context(self) -> dict[str, Any]:
        ...
        try:
            ...
            if metrics:
                total_memory = sum(comp.get("memory_usage", {}).get("current", 0) for comp in metrics.values())

                if total_memory > 50:  # > 50MB total
                    context["high_memory_usage"] = True

                # Add response time context
                avg_response_time = sum(comp.get("response_time", {}).get("average", 0) for comp in metrics.values()) / len(metrics) if metrics else 0
            ...
            return {"timestamp": time.time(), "health_score": 100, "active_alerts": 0, "monitoring_unavailable": True}

    def _extract_component_from_exception(self, exception: Exception) -> str | None:
    ...
    async def _add_auto_fix_suggestions(self, error: UserFriendlyError, exception: Exception, context: dict[str, Any] = None) -> UserFriendlyError:
        ...
        if error.error_code in {"SYS-001", "PERF-001"}:
            ...
            enhanced_actions.extend(["🔧 Automated memory cleanup is available", "System can clear caches and free up memory automatically", "Restart of memory-intensive components can be automated"])
        elif error.error_code == "NET-001":
            enhanced_actions.extend(
                ["🔧 Network connectivity tests can be run automatically", "System can check firewall rules and restart network services", "Automated retry with exponential backoff is available"]
            )
        elif error.error_code == "DB-001":
            enhanced_actions.extend(["🔧 Database connection recovery is automated", "System will automatically retry failed operations", "Connection pool optimization can be applied automatically"])
        elif error.error_code == "CFG-001":
            enhanced_actions.extend(
                ["🔧 Configuration validation and repair is available", "System can automatically restore valid configuration", "Backup configurations can be applied automatically"]
            )
    ...
    async def _log_error_for_monitoring(self, error: UserFriendlyError, exception: Exception) -> None:
    ...
    async def _enhance_with_system_health(self, context: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            dashboard_data = await self.monitoring._prepare_dashboard_data()
            context["system_health"] = {"score": dashboard_data.get("health_score", 100), "status": "healthy" if dashboard_data.get("health_score", 100) > 80 else "degraded"}
        except Exception:
    ...
    async def _enhance_with_performance_metrics(self, context: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            if metrics:
                ...
                avg_response_time = sum(comp.get("response_time", {}).get("average", 0) for comp in metrics.values()) / total_components if total_components > 0 else 0

                total_memory = sum(comp.get("memory_usage", {}).get("current", 0) for comp in metrics.values())

                context["performance"] = {"avg_response_time": avg_response_time, "total_memory_mb": total_memory, "components_monitored": total_components}
        except Exception:
    ...
    async def _enhance_with_component_status(self, context: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            for component, comp_metrics in metrics.items():
                ...
                component_status[component] = {"status": status, "response_time": rt, "memory_usage": mem}
    ...
    def get_error_statistics(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {"error": "Statistics unavailable", "last_updated": time.time()}
    ...
    async def test_error_handling(self, error_type: str = "generic") -> UserFriendlyError:
        ...
        return await self.handle_error(exception, context={"test": True, "error_type": error_type}, request_id=f"test-{int(time.time())}")
