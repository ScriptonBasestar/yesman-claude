...
class CanaryDeploymentManager:
    ...
    def __init__(self, config: Optional[CanaryConfig] = None):
        ...
        self._ramp_up_task: Optional[asyncio.Task] = None
    ...
    async def start_canary_deployment(self, deployment_id: str, config: Optional[CanaryConfig] = None) -> dict[str, Any]:
        ...
        if self.active_deployment:
            raise RuntimeError(f"Canary deployment {self.active_deployment.deployment_id} is already active")
        ...
        try:
            ...
            self._monitoring_task = asyncio.create_task(
                self._monitor_canary_deployment(deployment)
            )
            
            # Start ramp-up task if configured
            if deployment_config.ramp_up_steps:
                self._ramp_up_task = asyncio.create_task(
                    self._ramp_up_traffic(deployment)
                )
            ...
            await self._publish_deployment_event("deployment_failed", {
                'deployment_id': deployment_id,
                'error': str(e),
                'duration_seconds': deployment.completion_time - deployment.start_time
            })
            
            raise RuntimeError(f"Failed to start canary deployment: {e}")
    ...
    async def _capture_baseline_metrics(self) -> CanaryMetrics:
        ...
        try:
            ...
            if dashboard_data.get('metrics'):
                ...
                if total_samples > 0:
                    avg_response_time = avg_response_time / total_samples
    ...
    async def _configure_canary_traffic(self, percentage: int) -> None:
        ...
        await self._publish_deployment_event("traffic_configured", {
            'traffic_percentage': percentage,
            'previous_percentage': getattr(self, '_previous_traffic_percentage', 0),
    ...
    async def _monitor_canary_deployment(self, deployment: CanaryDeployment) -> None:
        ...
        try:
            while time.time() < end_time and deployment.status == DeploymentStatus.MONITORING:
                ...
                await self._publish_deployment_event("monitoring_update", {
            ...
            deployment.status = DeploymentStatus.FAILED
            deployment.completion_time = time.time()
            await self._publish_deployment_event("monitoring_cancelled", {
                'deployment_id': deployment.deployment_id
            })
        except Exception as e:
            deployment.status = DeploymentStatus.FAILED
            deployment.completion_time = time.time()
            await self._publish_deployment_event("monitoring_error", {
                'deployment_id': deployment.deployment_id,
                'error': str(e)
            })
    ...
    async def _evaluate_canary_health(self, deployment: CanaryDeployment, current_metrics: CanaryMetrics) -> dict[str, Any]:
        ...
        if current_metrics.error_rate > config.max_error_rate:
            ...
            evaluation['details']['error_rate_check'] = {
                'current': current_metrics.error_rate,
                'threshold': config.max_error_rate,
                'exceeded_by': current_metrics.error_rate - config.max_error_rate
            }
        
        evaluation['checks'].append({
            'name': 'error_rate',
            'passed': current_metrics.error_rate <= config.max_error_rate,
            'current': current_metrics.error_rate,
            'threshold': config.max_error_rate
        })
        ...
        evaluation['checks'].append({
            'name': 'success_rate',
            'passed': current_metrics.success_rate >= config.success_threshold,
            'current': current_metrics.success_rate,
            'threshold': config.success_threshold
        })
        ...
        evaluation['checks'].append({
            'name': 'health_score',
            'passed': current_metrics.health_score >= config.health_score_threshold,
            'current': current_metrics.health_score,
            'threshold': config.health_score_threshold
        })
        ...
        if (current_metrics.resource_utilization.get('cpu_percent', 0) > cpu_threshold or
            current_metrics.resource_utilization.get('memory_percent', 0) > memory_threshold):
            evaluation['healthy'] = False
            if not evaluation['reason']:
        ...
        evaluation['checks'].append({
            ...
            'passed': (current_metrics.resource_utilization.get('cpu_percent', 0) <= cpu_threshold and
                      current_metrics.resource_utilization.get('memory_percent', 0) <= memory_threshold),
            'cpu_percent': current_metrics.resource_utilization.get('cpu_percent', 0),
            'memory_percent': current_metrics.resource_utilization.get('memory_percent', 0)
        })
    ...
    async def _rollback_deployment(self, deployment: CanaryDeployment, reason: RollbackReason) -> None:
        ...
        try:
            ...
            await self._publish_deployment_event("deployment_rolled_back", {
                'deployment_id': deployment.deployment_id,
            ...
            await self._publish_deployment_event("rollback_error", {
                'deployment_id': deployment.deployment_id,
                'rollback_reason': reason.value,
                'error': str(e)
            })
    ...
    async def _promote_canary_deployment(self, deployment: CanaryDeployment) -> None:
        ...
        try:
            ...
            await self._publish_deployment_event("deployment_promoted", {
                'deployment_id': deployment.deployment_id,
            ...
            await self._publish_deployment_event("promotion_error", {
                'deployment_id': deployment.deployment_id,
                'error': str(e)
            })
    ...
    async def _ramp_up_traffic(self, deployment: CanaryDeployment) -> None:
        ...
        try:
            for step_percentage in deployment.config.ramp_up_steps:
                ...
                await self._publish_deployment_event("traffic_ramped_up", {
                    'deployment_id': deployment.deployment_id,
                    'new_traffic_percentage': step_percentage,
                    'ramp_up_time': time.time()
                })
                
        except asyncio.CancelledError:
            pass
        except Exception as e:
            await self._publish_deployment_event("ramp_up_error", {
                'deployment_id': deployment.deployment_id,
                'error': str(e)
            })
    ...
    def _load_deployment_history(self) -> None:
        ...
        if self.deployment_data_path.exists():
            try:
                with open(self.deployment_data_path) as f:
                    data = json.load(f)
    ...
    async def _save_deployment_state(self) -> None:
        ...
        try:
            ...
            state_data = {
                ...
                } if self.active_deployment else None,
                'deployment_history_count': len(self.deployment_history),
                'last_updated': time.time()
            }
            
            with open(self.deployment_data_path, 'w') as f:
    ...
    async def _publish_deployment_event(self, event_subtype: str, data: dict[str, Any]) -> None:
        ...
        await self.event_bus.publish(
            Event(
                ...
                data={
                    'event_subtype': f'canary_{event_subtype}',
                    **data
                },
                timestamp=time.time(),
    ...
    async def manual_rollback(self, reason: str = "Manual intervention") -> dict[str, Any]:
        ...
        if not self.active_deployment:
            return {
                'success': False,
                'error': 'No active deployment to rollback'
            }
    ...
    async def get_deployment_status(self) -> dict[str, Any]:
        ...
        return {
            'active_deployment': {
                ...
                } if deployment.current_metrics else None,
                'baseline_metrics': {
                ...
                } if deployment.baseline_metrics else None,
