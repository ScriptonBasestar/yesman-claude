...
class ContextDetector:
    ...
    def __init__(self, project_path: Optional[Path] = None):
        ...
        self.logger = logging.getLogger(f"yesman.context_detector")
            ...
            try:
                for file_path in self.project_path.glob(f"**/{pattern}"):
                    if file_path.is_file():
                        ...
                        if file_key in self._last_file_mtimes:
                            if current_mtime > self._last_file_mtimes[file_key]:
                                # File was modified
                                detected_changes.append(ContextInfo(
    ...
    def _calculate_confidence(self, context_type: ContextType, content: str, match: re.Match) -> float:
        ...
        if context_type == ContextType.TEST_FAILURE:
            if "failed" in match.group().lower() and any(test_word in content.lower() 
        ...
        elif context_type == ContextType.BUILD_FAILURE:
            if "error" in match.group().lower() and any(build_word in content.lower()
