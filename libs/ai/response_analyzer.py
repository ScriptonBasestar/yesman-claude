...
class ResponseAnalyzer:
    ...
    def record_response(self, prompt_text: str, user_response: str, 
    ...
    def _update_patterns(self, record: ResponseRecord):
        ...
        if record.context:
            ...
            pattern.context_factors[context_key] = min(1.0, 
                pattern.context_factors[context_key] + 0.1)
    ...
    def predict_response(self, prompt_text: str, context: str = "", 
                        project_name: str = None) -> Tuple[str, float]:
        ...
        if pattern.common_responses:
            best_response = max(pattern.common_responses.items(), 
                              key=lambda x: x[1])[0]
            return best_response, confidence
        else:
            return self._get_default_response(prompt_type)
            
    def _calculate_confidence(self, pattern: PromptPattern, 
                            prompt_text: str, context: str) -> float:
