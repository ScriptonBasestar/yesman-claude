...
class TaskRunner:
    ...
    def find_todo_files(self, specific_dir: Optional[str] = None) -> List[Path]:
        ...
        if specific_dir:
            ...
            if specific_dir.startswith('/'):
                search_dir = Path(specific_dir)
            else:
                search_dir = self.todo_dir / specific_dir
        else:
    ...
    def analyze_task_dependencies(self, task: TodoTask) -> List[str]:
        ...
        complex_keywords = [
            'implement', 'create system', 'build', 'design', 
            'integrate', 'optimize', 'refactor'
        ]
    ...
    def run_tests(self) -> bool:
        ...
        try:
            ...
            result = subprocess.run(['python', '-m', 'pytest', '--tb=short'], 
                                  capture_output=True, text=True, cwd=self.todo_dir.parent)
