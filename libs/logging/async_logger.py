    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        stats = {
            **self.stats,
            ...
            'queue_utilization': self.log_queue.qsize() / self.config.max_queue_size,
...
async def get_async_logger(config: AsyncLoggerConfig = None) -> AsyncLogger:
    """Get or create the global async logger instance."""
    global _global_async_logger
    
    if _global_async_logger is None:
        _global_async_logger = AsyncLogger(config)
        await _global_async_logger.start()
...
async def stop_global_logger():
    ...
    if _global_async_logger:
        await _global_async_logger.stop()
        _global_async_logger = None


# Convenience functions for quick logging
def async_log(level: LogLevel, message: str, **kwargs):
    """Queue a log message to the global async logger."""
    global _global_async_logger
    
    if _global_async_logger:
        _global_async_logger.log(level, message, **kwargs)
    else:
        # Fallback to standard logging
        logging.getLogger("yesman_async_fallback").log(level.level_value, message)


def async_info(message: str, **kwargs):
    """Log info message to global async logger."""
    async_log(LogLevel.INFO, message, **kwargs)


def async_debug(message: str, **kwargs):
    """Log debug message to global async logger."""
    async_log(LogLevel.DEBUG, message, **kwargs)


def async_warning(message: str, **kwargs):
    """Log warning message to global async logger."""
    async_log(LogLevel.WARNING, message, **kwargs)


def async_error(message: str, **kwargs):
    """Log error message to global async logger."""
    async_log(LogLevel.ERROR, message, **kwargs)