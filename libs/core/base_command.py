...
class BaseCommand(ABC):
    ...
    def __init__(self, 
                 config: Optional[YesmanConfig] = None,
                 tmux_manager: Optional[TmuxManager] = None,
                 claude_manager: Optional[ClaudeManager] = None):
        """
    ...
    def _create_tmux_manager(self) -> TmuxManager:
        ...
        try:
            ...
            raise CommandError(f"Tmux manager error: {e}")
    ...
    def _create_claude_manager(self) -> ClaudeManager:
        ...
        try:
            ...
            raise CommandError(f"Claude manager error: {e}")
...
class ConfigCommandMixin:
    ...
    def load_projects_config(self) -> Dict[str, Any]:
        ...
        try:
            import yaml
            with open(settings.paths.projects_file, 'r') as f:
                return yaml.safe_load(f) or {}
        except FileNotFoundError:
            self.logger.warning(f"Projects file not found: {settings.paths.projects_file}")
            return {}
        except Exception as e:
            raise CommandError(f"Failed to load projects configuration: {e}")
    ...
    def save_projects_config(self, config: Dict[str, Any]) -> None:
        ...
        try:
            ...
            raise CommandError(f"Failed to save projects configuration: {e}")
