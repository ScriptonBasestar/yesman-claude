...
class SessionValidator:
    ...
    def _validate_session_name(self, session_name: str) -> bool:
        ...
        if len(session_name) > settings.sessions.session_name_max_length:
            self.validation_errors.append(
                f"Session name '{session_name}' too long "
                f"(max {settings.sessions.session_name_max_length} characters)"
            )
            return False
    ...
    def _validate_windows(self, session_name: str, config_dict: Dict[str, Any]) -> bool:
        ...
        if len(windows) > settings.sessions.max_windows_per_session:
            self.validation_errors.append(
                f"Too many windows ({len(windows)}) for session '{session_name}' "
                f"(max {settings.sessions.max_windows_per_session})"
            )
            return False
        
        for i, window in enumerate(windows):
            if not self._validate_window(session_name, i, window, config_dict):
                return False
        
        return True
    
    def _validate_window(self, session_name: str, window_index: int, 
                        window: Dict[str, Any], config_dict: Dict[str, Any]) -> bool:
        ...
        if window_start_dir:
            if not self._validate_window_start_directory(
                session_name, window_name, window_start_dir, config_dict
            ):
                return False
        ...
        if len(panes) > settings.sessions.max_panes_per_window:
            self.validation_errors.append(
                f"Too many panes ({len(panes)}) in window '{window_name}' "
                f"(max {settings.sessions.max_panes_per_window})"
            )
            return False
        
        return True
    
    def _validate_window_start_directory(self, session_name: str, window_name: str,
                                       window_start_dir: str, config_dict: Dict[str, Any]) -> bool:
        """Validate window start directory"""
...
class SessionConfigBuilder:
    ...
    def _load_template(self, template_name: Optional[str]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            raise CommandError(f"Failed to read template {template_file}: {e}")
...
class SessionSetupService:
    ...
    def setup_sessions(self, session_filter: Optional[str] = None) -> Tuple[int, int]:
        """
    ...
    def _load_sessions_config(self, session_filter: Optional[str] = None) -> Dict[str, Any]:
    ...
    def _kill_session(self, session_name: str) -> None:
        ...
        try:
            ...
            raise CommandError(f"Failed to kill existing session: {e}")
    ...
    def _create_session(self, config_dict: Dict[str, Any]) -> None:
        ...
        try:
            ...
            raise CommandError(f"Failed to create tmux session: {e}")