...
from .types import TaskStatus, AgentState
...
class SuccessCriteria:
    ...
    def check_compliance(self, metrics: PerformanceMetrics) -> Dict[str, bool]:
        ...
        results = {
            "speed_improvement": (
                self.min_speed_improvement <= metrics.speed_improvement_ratio <= self.max_speed_improvement
            ),
...
class MetricsVerifier:
    ...
    async def measure_multi_agent_performance(
        ...
        if self.current_metrics.single_agent_time > 0:
            self.current_metrics.speed_improvement_ratio = (
                self.current_metrics.single_agent_time / self.current_metrics.multi_agent_time
            )
    ...
    def track_conflict_resolution(
        ...
        logger.info(
            f"Conflict resolution: {auto_resolved}/{total_conflicts} "
            f"(rate: {self.current_metrics.conflict_resolution_rate:.2%})"
        )
        self._save_metrics()
    ...
    def track_merge_success(
        ...
        logger.info(
            f"Merge success: {successful}/{total_attempts} "
            f"(rate: {self.current_metrics.merge_success_rate:.2%})"
        )
        self._save_metrics()
    ...
    def track_code_quality(
        ...
        logger.info(
            f"Code quality: {initial_score:.2f} -> {final_score:.2f} "
            f"(change: {self.current_metrics.quality_improvement:+.2f})"
        )
        self._save_metrics()
    ...
    def generate_performance_report(self) -> str:
        ...
        report_lines = [
            ...
            "",
            "🚀 Speed Improvement:",
            f"  Target: 2.0x - 3.0x improvement",
            ...
            "",
            "🔧 Conflict Resolution:",
            f"  Target: ≥80% auto-resolution rate",
            ...
            "",
            "📈 Code Quality:",
            f"  Target: No degradation (≥0.0 improvement)",
