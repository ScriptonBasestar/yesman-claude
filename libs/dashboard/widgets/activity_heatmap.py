...
class ActivityHeatmapGenerator:
                ...
                for line in f:
                    ...
                    if match:
                        ...
                        try:
                            ...
                            continue
            logger.info(f"Collected activity for {session_name} in {(datetime.now(UTC) - start_time_collect).total_seconds():.4f} seconds.")
            return activity_counts
        except Exception:
            logger.exception("Error collecting session activity for {session_name}")
            return {}
    ...
    def generate_heatmap_data(self, sessions: list[str], days: int = 7) -> dict[str, Any]:
        ...
        logger.info(f"Generated heatmap data for sessions {sessions} in {(datetime.now(UTC) - start_time_generate).total_seconds():.4f} seconds.")
        return {
