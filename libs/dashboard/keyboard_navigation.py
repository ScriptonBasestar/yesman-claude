...
class KeyBinding:
    ...
    def matches(self, key: str, modifiers: List[KeyModifier]) -> bool:
        ...
        return (
            ...
            set(self.modifiers) == set(modifiers) and
            self.enabled
        )
...
class KeyboardNavigationManager:
    ...
    def unregister_binding(self, key: str, modifiers: List[KeyModifier], context: Optional[NavigationContext] = None) -> bool:
        ...
        if context is None:
            ...
            self.bindings[key_combo] = [
                b for b in self.bindings[key_combo] 
    ...
    def handle_key_event(self, key: str, modifiers: List[KeyModifier], context: Optional[NavigationContext] = None) -> bool:
        ...
        for binding in self.bindings[key_combo]:
            ...
            # Check context match
            if binding.context is None or binding.context == context or binding.context == NavigationContext.GLOBAL:
    ...
    def import_bindings(self, bindings_data: Dict[str, Any]) -> None:
        ...
        for key_combo, binding_list in bindings_data.items():
            self.bindings[key_combo] = [
                ...
                for binding_data in binding_list
            ]
    ...
    def get_help_text(self, context: Optional[NavigationContext] = None) -> List[str]:
        ...
        Args:
            context: Context to get help for (None for current context)
        ...
        for key_combo, bindings in sorted(self.bindings.items()):
            for binding in bindings:
                if not binding.enabled or binding.action in seen_actions:
                    continue
                
                if binding.context is None or binding.context == context or binding.context == NavigationContext.GLOBAL:
                    help_lines.append(f"{key_combo:<20} {binding.description}")
...
def get_keyboard_manager() -> KeyboardNavigationManager:
    ...
    if _global_keyboard_manager is None:
        _global_keyboard_manager = KeyboardNavigationManager()
    return _global_keyboard_manager


def reset_keyboard_manager() -> None:
    """Reset the global keyboard navigation manager"""
    global _global_keyboard_manager
    _global_keyboard_manager = None