...
test-slow: ## run slow tests only
	@echo "$(CYAN)Running slow tests only...$(RESET)"
	python -m pytest tests/ -m "slow" $(PYTEST_OPTS)
	@echo "$(GREEN)✅ Slow tests completed$(RESET)"

test-failed: ## run only previously failed tests
	@echo "$(CYAN)Running previously failed tests...$(RESET)"
	python -m pytest tests/ --lf $(PYTEST_OPTS)
	@echo "$(GREEN)✅ Failed tests re-run completed$(RESET)"
...
cover-report: ## show detailed coverage report
	...
	@echo ""
	@echo "$(BLUE)For HTML report, run: make cover-html$(RESET)"

cover-xml: ## generate XML coverage report (for CI)
	@echo "$(CYAN)Generating XML coverage report...$(RESET)"
	python -m pytest tests/ $(PYTEST_OPTS) --cov=libs --cov=commands --cov-report=xml
	@echo "$(GREEN)✅ XML coverage report generated$(RESET)"

cover-check: ## check if coverage meets minimum threshold
	@echo "$(CYAN)Checking coverage threshold...$(RESET)"
	@python -m coverage report --fail-under=80 || \
		(echo "$(RED)❌ Coverage below 80% threshold$(RESET)" && exit 1)
	@echo "$(GREEN)✅ Coverage meets threshold$(RESET)"
...
test-marker: ## run tests with specific marker (MARKER=slow)
	...
	@echo "$(CYAN)Running tests with marker: $(MARKER)$(RESET)"
...
test-keyword: ## run tests matching keyword (KEYWORD=prompt)
	...
	@echo "$(CYAN)Running tests matching keyword: $(KEYWORD)$(RESET)"
