...
def main():
    ...
    success1, stdout1, stderr1 = run_command(
        ["python", "-m", "ruff", "check", "libs", "commands", "api", "tests", 
         "--diff", "--exclude", "migrations", "--exclude", "node_modules", "--exclude", "examples"],
        "Ruff check with diff"
    )
    
    # 2. MyPy check
    success2, stdout2, stderr2 = run_command(
        ["python", "-m", "mypy", "libs", "commands", "api", 
         "--ignore-missing-imports", "--exclude", "migrations", "--exclude", "node_modules", "--exclude", "examples"],
        "MyPy type checking"
    )
    
    # 3. Bandit security check
    success3, stdout3, stderr3 = run_command(
        ["python", "-m", "bandit", "-r", "libs", "commands", "api", 
         "--skip", "B101,B404,B603,B607,B602", "--severity-level", "medium", "--quiet",
    ...
    success4, stdout4, stderr4 = run_command(
        ["python", "-m", "mdformat", "--check", "--diff", "*.md", "--wrap", "120"],
        "MDFormat check"
    )
