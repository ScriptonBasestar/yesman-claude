...
class TestAILearningIntegration(AsyncIntegrationTestBase):
    ...
    def _setup_claude_responses(self) -> None:
        ...
        self.mock_claude.add_mock_response(
            "write a function", "def example_function():\n    return 'Hello World'"
        )
        ...
        self.mock_claude.add_mock_response(
            "create a task", "I'll help you create a task. What's the task description?"
        )
    ...
    async def test_learning_from_session_interactions(self) -> None:
        ...
        for interaction in interaction_patterns:
            claude_response = self.mock_claude.simulate_interaction(
                interaction["prompt"]
            )
        ...
        coding_patterns = [
            p for p in detected_patterns if "coding" in p.get("category", "").lower()
        ]
        ...
        explanation_patterns = [
            ...
            if "explanation" in p.get("category", "").lower()
        ]
        ...
        ), f"Learning phase took {learning_duration:.2f}s, should be < 5s"
        assert (
            detection_duration < 2.0
        ), f"Pattern detection took {detection_duration:.2f}s, should be < 2s"
    ...
    async def test_prediction_accuracy_improvement(self) -> None:
        ...
        improved_prediction = await learning_engine.predict_response_context(
            test_prompt
        )

        # Verify improvement
        assert (
            improved_prediction is not None
        ), "No prediction generated after extended training"

        # Confidence should improve with more data
        initial_confidence = (
            initial_prediction.get("confidence", 0.0) if initial_prediction else 0.0
        )
        improved_confidence = improved_prediction.get("confidence", 0.0)

        assert (
            improved_confidence > initial_confidence
        ), f"Confidence didn't improve: {initial_confidence} -> {improved_confidence}"
        assert (
            improved_confidence > 0.7
        ), f"Final confidence {improved_confidence} should be > 0.7"
    ...
    async def test_cross_session_learning(self) -> None:
        ...
        global_patterns = await learning_engine.detect_patterns(
            session_id=None
        )  # All sessions

        # Should detect web development pattern across sessions
        web_dev_patterns = [
            p for p in global_patterns if "web" in p.get("category", "").lower()
        ]
        assert (
            len(web_dev_patterns) > 0
        ), "No web development patterns detected across sessions"
        ...
        prediction = await learning_engine.predict_response_context(
            test_prompt, session_id=None
        )

        assert prediction is not None, "No cross-session prediction generated"
        assert (
            prediction.get("confidence", 0.0) > 0.6
        ), "Cross-session prediction confidence should be > 0.6"
...
class TestAutomationLearningIntegration(AsyncIntegrationTestBase):
    ...
    async def test_context_detection_learning(self) -> None:
        ...
        initial_result = self.command_runner.run_command(
            AutomateDetectCommand, project_path=str(project_dir)
        )

        initial_duration = self.performance_monitor.end_timing(
            "initial_context_detection"
        )
        ...
        improved_result = self.command_runner.run_command(
            AutomateDetectCommand, project_path=str(project_dir)
        )

        improved_duration = self.performance_monitor.end_timing(
            "improved_context_detection"
        )
        ...
        assert len(improved_contexts) >= len(
            initial_contexts
        ), "Learning didn't improve context detection"

        # Performance should be reasonable
        assert (
            initial_duration < 3.0
        ), f"Initial detection took {initial_duration:.2f}s, should be < 3s"
        assert (
            improved_duration < 3.0
        ), f"Improved detection took {improved_duration:.2f}s, should be < 3s"
    ...
    async def test_workflow_optimization_learning(self) -> None:
        ...
        recommendations = await learning_engine.get_workflow_recommendations(
            project_path=str(project_dir), context="python_project"
        )
        ...
        if run_test_recs:
            assert (
                run_test_recs[0].get("estimated_time", 999) < 3.0
            ), "Test execution time not optimized"
...
class TestLearningPersistenceIntegration(AsyncIntegrationTestBase):
    ...
    async def test_learning_data_persistence(self) -> None:
        ...
        learning_engine_1 = LearningEngine(
            config=self.get_test_config(), session_name=session_name
        )
        ...
        learning_engine_2 = LearningEngine(
            config=self.get_test_config(), session_name=session_name
        )
        ...
        deployment_patterns = [
            p for p in loaded_patterns if "deployment" in p.get("category", "").lower()
        ]
        ...
        assert (
            prediction.get("confidence", 0.0) > 0.5
        ), "Low confidence from persisted patterns"
    ...
    async def test_incremental_learning_persistence(self) -> None:
        ...
        ), "Incremental learning didn't add patterns"
        ...
        assert (
            len(reloaded_patterns) == updated_count
        ), "Incremental patterns not persisted correctly"
