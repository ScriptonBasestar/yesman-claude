    ...
    def create_chaos_scenario(scenario_type: str = None) -> Dict[str, Any]:
        ...
        if not scenario_type:
            scenario_type = random.choice([
                'network_failure', 'memory_pressure', 'cpu_saturation',
                'disk_full', 'dependency_unavailable', 'process_crash'
            ])
        
        scenario = {
            'scenario_id': fake.uuid4(),
            'scenario_type': scenario_type,
            'target_component': random.choice([
                'claude_monitor_async', 'session_manager', 'event_bus',
                'dashboard_integration', 'api_server'
            ]),
...
class PerformanceAlertFactory(MagicMock):
    ...
    def create_alert(cls, severity: str = 'warning', **kwargs) -> MagicMock:
        ...
        """
        alert = cls()
        alert.timestamp = kwargs.get('timestamp', datetime.now(timezone.utc).timestamp())
        ...
        alert.context = kwargs.get('context', {
...
class ChaosTestRunner:
    ...
    def create_resilience_test_matrix() -> Dict[str, List[Dict[str, Any]]]:
        ...
        return {
            'network_resilience': [
                EnhancedTestDataFactory.create_chaos_scenario('network_failure')
                for _ in range(5)
            ],
            'resource_resilience': [
                EnhancedTestDataFactory.create_chaos_scenario('memory_pressure')
                for _ in range(3)
            ] + [
                EnhancedTestDataFactory.create_chaos_scenario('cpu_saturation')
                for _ in range(2)
            ],
            'dependency_resilience': [
                EnhancedTestDataFactory.create_chaos_scenario('dependency_unavailable')
                for _ in range(4)
            ],
            'process_resilience': [
                EnhancedTestDataFactory.create_chaos_scenario('process_crash')
                for _ in range(3)
            ],
        }
